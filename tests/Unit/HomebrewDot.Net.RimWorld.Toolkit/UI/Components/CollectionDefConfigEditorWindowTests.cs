...
{
    [Trait("Category", "Unit")]
    public class CollectionDefConfigEditorWindowTests
    {
        [Fact]
        public void Constructor_WithNullConfig_ThrowsTypeInitializationOrSucceeds()
        {
            var exception = Record.Exception(() => new CollectionDefConfigEditorWindow(null, _ => { }));
        ...
        {
            var config = new CollectionDefConfig();
            var exception = Record.Exception(() => new CollectionDefConfigEditorWindow(config, _ => { }));
            if (exception != null)
            {
                Assert.True(
                    exception is TypeInitializationException ||
        ...
        public void Constructor_WithNullOnSave_ThrowsTypeInitializationOrArgumentNullException()
        {
            ...
            var exception = Record.Exception(() => new CollectionDefConfigEditorWindow(new CollectionDefConfig(), null));
            if (exception != null)
            {
                Assert.True(
                    exception is TypeInitializationException ||
        ...
        [Fact]
        public void Constructor_InitializesEmptyLists_WhenConfigHasNullLists()
        {
            var config = new CollectionDefConfig
            {
                Conditions = null,
