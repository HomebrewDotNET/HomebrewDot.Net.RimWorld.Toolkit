---
name: homelab-model-routing
description: 'Route to the most cost-effective OpenRouter model for homelab infrastructure agent delegation. Use when selecting a model for Planner, Implementer, or Debugging work based on task complexity. Returns a deterministic model recommendation as JSON.'
---

# Homelab Model Routing

## Purpose

Deterministic routing logic that recommends the single most cost-effective (cheapest) model based on two inputs: **Category** and **Complexity**. Used by orchestrator agents before delegating planning or implementation work to sub-agents for homelab infrastructure tasks (Puppet, PowerShell, Bash, Docker, node configuration).

## Input Parameters

| Parameter | Values | Description |
|-----------|--------|-------------|
| **Category** | `Planner`, `Implementer`, `Debugging` | The type of work being delegated |
